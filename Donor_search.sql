...
-- На первом месте крупные города, однако также много донаций без указания места, что необходимо исправить

-- 2) Изучаем динамику общего количества донаций в месяц за 2022 и 2023 гг. 
...
-- 3) Определяем наиболее активных доноров в системе, учитывая только данные о зарегистрированных и подтвержденных донациям.
SELECT id,
       confirmed_donations
FROM donorsearch.user_anon_data
...
-- 4) Оцениваем, как система бонусов влияет на зарегистрированные в системе донации
...
-- Всего 21 108 доноров получили бонусы, что значительно меньше по сравнению с общей базой доноров (256 491). 
--Это указывает на потенциал расширения программ лояльности для охвата большего числа доноров.
...
-- Исследование вовлечения новых доноров через социальные сети,
--  учитывая только тех, кто совершил хотя бы одну донацию. Таким образом узнаем, сколько 
...
-- Cравнение активности однократных доноров со средней активностью повторных доноров.

-- Группировка по времени: Рассмотрим, как изменяется активность доноров в зависимости от года первой донации.
-- Анализ частоты донаций: Разделим повторных доноров по количеству донаций.
...
SELECT first_donation_year,
       ...
       ROUND(AVG(total_donations),0) AS avg_donations_per_donor,
...
-------------------+------------------------+-----------+-----------------------+--------------------------+--------------------------+------------------------------+
                201|6 и более донаций       |          1|                     26|                 663670.00|                  26546.00|                       1824.00|
                207|6 и более донаций       |          1|                     37|                 661775.00|                  18382.00|                       1817.00|
                208|6 и более донаций       |          1|                      7|                 660907.00|                 110151.00|                       1817.00|
                214|6 и более донаций       |          1|                     39|                 658841.00|                  17337.00|                       1810.00|
               1019|6 и более донаций       |          1|                     33|                 366097.00|                  11440.00|                       1005.00|
               1900|6 и более донаций       |          1|                      7|                  45136.00|                   7522.00|                        125.00|
**/
...
-- Анализ планирований доноров и их реальной активности
